...
import {setLanguageFilterAction} from '../redux/filters/filterActions';
...
const LangFilter = (props: Props) => {
  let LangfilterLabel = useSelector(
    (state: any) => state.filters.LanguageFilterLabel,
  );
  ...
  useEffect(() => {
    ...
    let newList = LangsData.filter((item: string) =>
  ...
  function setLang(item: string) {
    if (item == 'All') {
      dispatch(setLanguageFilterAction('All', ''));
    } else {
      if (item == 'Unkown languages') {
        dispatch(setLanguageFilterAction('Unkown languages', null));
      } else {
        dispatch(setLanguageFilterAction(item, item));
      }
    }
  }
        ...
        <View style={styles.ModalCont}>
          ...
          <View style={styles.SearchBarCont}>
            <TextInput
              style={
                colorSchema === 'light'
                  ? [styles.SearchBar, stylesGlobal.lg, stylesGlobal.TextBody]
                  : [
                      styles.SearchBar,
                      stylesGlobal.lg,
                      stylesGlobal.DarkTextBody,
                    ]
              }
              onChangeText={setText}
              value={text}
              placeholderTextColor={
                colorSchema === 'light' ? '#AEB7C2' : '#6E767D'
              }
            ...
            <Image
              ...
              source={require('../assets/searchF.png')}
            />
          </View>
